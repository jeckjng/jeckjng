...
class PageController extends HomebaseController{
	...
	/* 数据库域名替换 */
   public function test11111111111111111() {
       $userinfo=M("users")->field("id,avatar")->select();
			 foreach($userinfo as $k=>$v){
				    if(strstr($v['avatar'],'http://api.phpweb.cn:8080/' )){
							
							  $newavatar=str_replace("http://api.phpweb.cn:8080/","http://livenew.youyuzhibo.com/public/",$v['avatar']);
								
								M("users")->where("id='{$v['id']}'")->save( array("avatar"=>$newavatar) );
