...
//					->fetchOne();
	...
	function getLevel($experience){
		...
		$level=getcaches($key);
		if(!$level){
			$level= M("experlevel")->order("experience asc")->select();
			setcaches($key,$level);			 
		}
	...
	function getLevelAnchor($experience){
		...
		$level=getcaches($key);
		if(!$level){
			$level= M("experlevel_anchor")->order("experience asc")->select();
			setcaches($key,$level);			 
		}
    ...
    function getUserInfo($uid) {
        $info= M("users")->field("id,user_nicename,avatar,avatar_thumb,sex,signature,consumption,votestotal,province,city,birthday,issuper,pids")->where("id='{$uid}'")->find();
		if($info){
   ...
   */
	...
	*/
	...
	function createCode(){
		...
		if(mb_strlen($d)==6){
			$oneinfo=M("users_agent_code")->field("code")->where("code='{$d}'")->find();
			if(!$oneinfo){
				return $d;
			}
		}
