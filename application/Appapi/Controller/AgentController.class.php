...
class AgentController extends HomebaseController {
	...
	function ewm(){ 
	    $url="http://www.qq.com/Appapi/Reg/reg?agentid=U3SXRY";
	    $url=shorturl($url);
	    exit($url);
		$uid=I("uid");
		$token=I("token"); 
        ...
        $url=shorturl("http://42.51.65.24/Appapi/Reg/reg?agentid=".$codeinfo['code']);
	...
	function index(){       
        ...
        $longurl=shorturl("http://live.ssuzz.cc/Appapi/Reg/reg?agentid=".$codeinfo['code']);
        //echo $longurl;exit;
		...
		if($users_agent){
            $path_a=explode(';',$users_agent['path']);
            $path_a=array_filter($path_a);
            $one_uid=(int)end($path_a);
			$agentinfo=$User->field("id,user_nicename,avatar")->where("id={$one_uid}")->find();
		}
	...
	function setAgent(){
        ...
        $isexist=M("users_agent")->where("uid={$uid}")->find();
		if($isexist){
            $rs['code']=1005;
			$rs['msg']='已设置';
			echo json_encode($rs);
