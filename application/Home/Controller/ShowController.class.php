...
class ShowController extends HomebaseController {
  //首页
	public function index(){
		...
		$anchorinfo=getTenantUserInfo($anchorid,$tenantId);
		...
		if($liveinfo['isvideo']==0){
            if($this->configpri['cdn_switch']!=5){
                ...
                $liveinfo['flvpull']=PrivateKeyA('http',$liveinfo['stream'].'.flv',0);
            }
		}
		...
		if($uid==$anchorinfo['id']){ //推流
            ...
            else if($this->configpri['cdn_switch']==2){
                $push=PrivateKeyA('rtmp',$stream,1);
                $push=$push['cdn'].$push['stream'];
            }
            else{
                $push=PrivateKeyA('rtmp',$stream,1);
            }
	...
	*/
	...
	public function createRoom(){
		...
		$User=M("users");
		$live=M("users_live");
		$userinfo= $User->field('coin,token,expiretime,user_nicename,avatar,avatar_thumb,tenant_id,isshare')->where("id='{$uid}'")->find();
		...
		else
		{
			...
			if($configpri['cdn_switch']==5)
			{
				...
				$flvpull=PrivateKeyA('http',$stream.'.flv',0);
			}
    ...
    public function stopRoom(){
		...
		$info=M('users_live')
				...
				->where("uid={$uid} and stream='{$stream}' and islive='1'")
				->find();
		if($info){
	...
	*/
	...
	public function endRecommend(){
		/* 推荐列表 */
        $tenantId=getTenantId();
		$list=M("users_live")->where("islive='1' and ( tenant_id='{$tenantId} or isshare='1' )")->order("rand()")->limit(0,4)->select();
		foreach($list as $k=>$v){
		...
		if($uid == 0 || $uid == '0' || $uid == ""){
			...
			if($uid == $anchorid){
				...
				if(isAttention($uid,$anchorid)){
                    ...
                    /**
                     * TODO 判断发起关注的用户和被关注用户的身份
                     * 不同租户的用户,在对方非公共主播时无法关注
                     */
    ...
    /*主播页面推流码弹窗*/
    public function pushStreamCode()
    {
        $push=session('push');

        $pushurl=substr($push,0,strrpos($push,'/')+1);
        $pushcode=substr($push,strrpos($push,'/')+1);

        $this->assign("pushurl",$pushurl);
        $this->assign("pushcode",$pushcode);
        $this->display();
    }
