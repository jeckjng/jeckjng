...
class Page {
    ...
    public function Pager_old($Page_tpl = '') {
        if (empty($Page_tpl))
    ...
    public function Pager($Page_tpl = '') {
        ...
        $replace = array($cfg ['recordcount'], $cfg ['pagecount'], $cfg ['pageindex'], $cfg ['pagesize'], $pList, $pListStart, $pListEnd, $pFirst, $pPrev, $pNext, $pLast, $pJump);
        $tmpStr = chr(13) . chr(10) . preg_replace($patterns, $replace, $tmpStr) . chr(13) . chr(10);
        $selected5 = $selected10 = $selected20 = $selected50 = $selected100 = $selected200 = '';
        if ($this->Page_size == 5){
            $selected5 = 'selected';
        }elseif ($this->Page_size == 10){
            $selected10 = 'selected';
        }elseif ($this->Page_size == 20){
            $selected20 = 'selected';
        }elseif ($this->Page_size == 50){
            $selected50 = 'selected';
        }elseif ($this->Page_size == 100){
            $selected100 = 'selected';
        }elseif ($this->Page_size == 200){
            $selected200 = 'selected';
        }
        unset($cfg);
        return "
			<select class='select_1' onchange=changePageNum(this) style='width: 120px;'>
			    <option value='".$this->urlNum(5,1)."' ".$selected5.">5条/页</option>
			    <option value='".$this->urlNum(10,1)."' ".$selected10.">10条/页</option>
				<option value='".$this->urlNum(20,1)."' ".$selected20.">20条/页</option>
				<option value='".$this->urlNum(50,1)."' ".$selected50.">50条/页</option>
				<option value='".$this->urlNum(100,1)."' ".$selected100.">100条/页</option>
				<option value='".$this->urlNum(200,1)."' ".$selected200.">200条/页</option>
			</select>
		{$tmpStr}";
    }
    ...
    private function urlNum($num,$page){
        $_GET['num'] = $num;
        $str = str_replace(urlencode('[PAGE]'), $page, $this->UrlParameters($_GET));
