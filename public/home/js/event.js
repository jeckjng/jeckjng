var JsInterface={
	...
	chatFromSocket: function(data) {
		...
		var tenantId= msgObject.tenantId;
	...
	sendBigHorn:function(data){
		...
		// 	},5000)
			if(player===null){
				//video.js+jquery danmu
				var time = $('#danmu').data("nowTime")+1;

				if(_DATA.user!=null && data.ct.uid==_DATA.user.id){
					//自己的弹幕
					$('#danmu').danmu("addDanmu",{text:data.ct.content,color:"white",position:0,time:time,size:1,isnew:1});
				}else{
					$('#danmu').danmu("addDanmu",{text:data.ct.content,color:"white",position:0,time:time,size:1});
				}
			}
			else{
				//aliplayer
				var danmu= player.getComponent("AliplayerDanmuComponent");
				var mode=randomNum(1,2);
				var danmuObj={
					"mode": mode,            // mode 表示弹幕的类型，参考 弹幕类型 https://github.com/jabbany/CommentCoreLibrary/blob/master/docs/CommentTypes.md
					"text": data.ct.content,       // text 表示弹幕的文字内容。注意：在创造弹幕对象后，对 text 的更改将无意义。
