...
class AdminbaseController extends AppframeController {
	...
	public function __construct() {
        ...
        $this->redis = connectionRedis();

        // 设置时区
        date_default_timezone_set('Asia/Shanghai');

        $this->assign('time_se', get_timeselect());
        $this->assign('time_zone', date_default_timezone_get());
        $this->assign('time_now', date('Y-m-d H:i:s'));
        $this->assign('auth_access', showHidden(U('setting/get_time_date_info')));
    ...
    function _initialize(){
       ...
       $this->load_app_admin_menu_lang();
    	if(isset($_SESSION['ADMIN_ID'])){
            ...
            if(($user['admin_type'] ==1 && ACTION_NAME == 'indexfamily') || ACTION_NAME == 'img_upload'){
...
//            $page_size = C("PAGE_LISTROWS");
    ...
    private function check_access($uid){
    	...
    	$no_need_check_rules=array("AdminIndexindex","AdminMainindex","AdminTenantConfigset_post","AdminPlatformConfigset_post");
    ...
    public function out_put($data, $code = 200, $msg = ''){
        $out_out = array(
            'code' => intval($code),
            'msg' => trim($msg),
            'data' => $data,
