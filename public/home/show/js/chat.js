...
var liveType=
{
	...
	pushStreamCode:function(){
		$.ajax({
			type:"post",
			url:"/index.php?g=home&m=Show&a=pushStreamCode",
			data:{},
			success:function(data)
			{
				document.getElementById('ds-dialog-bg').style.display='block';
				$("#pushStreamCode").css({"left":getMiddlePos('pushStreamCode').pl+"px","z-index":210,}).show();
				$("#pushStreamCode").html(data);
			}
		});
	},
	closePushStreamCode:function (){
		$('#pushStreamCode').hide();
		$('#pushStreamCode').html("");
	...
	other:function()
	{
		...
		var ctroom={"title":title,"type":type,"stand":stand,"stream":_DATA.anchor.stream,"uid":_DATA.user.id,"token": _DATA.user.token};

		JsInterface.createRoom(ctroom);
		//document.getElementById('webplayer')._myclock(choice);
	},
	...
	aliplayer:function(){
		//使用aliplayer
		console.log("使用aliplayer");
		var danmukuList = [];
		var pullUrl=_DATA.live.flvpull;
		console.log("播放地址",pullUrl);
		console.log(_DATA.live);
		// var danmuinput=$(".horn-send");
		var danmuinput=".horn-send";
 		...
 		$.ajax({
			...
			success:function(data){
				if(data.code==0){
					...
					var msg = '{"msg":[{"_method_":"ShutUpUser","action":"1","ct":"'+name+'被禁言了'+data.info.shut_time+'","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nicename+'","touid":'+touid+',"toname":"'+name+'","vip_type":"0","liangname":"0","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}],"retcode":"000000","retmsg":"ok"}';
					Socket.emitData('broadcast',msg);
				}else{
					layer.msg(data.msg);
				}
		...
		$.ajax({
			...
			success:function(data){
				if(data.ret==200 && data.data.code==0){
					 var msg = '{"msg":[{"_method_":"stopLive","action":"10","ct":"该直播间涉嫌违规，已被停播","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nicename+'","touid":'+_DATA.anchor.id+',"touname":"'+_DATA.anchor.id.user_nicename+'","vip_type":"0","liangname":"0","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}],"retcode":"000000","retmsg":"ok"} ';
					Socket.emitData('broadcast',msg);
...
var User={
	...
	popup:function(id){
		...
		clearTimeout(timeout); //timeout在模板上定义
		...
		var winHeight=$(window).height()-10; //删掉padding 10px
	...
	setUserHeight: function(){
		var _this=this;
		/*var height=$(window).height()-10; //删掉padding 10px
		if(height<minHeight ){
			height=minHeight;
		...
		var height=$(".MR-chat .boarder .clearfix").height();
		var h=height;
	...
	setAdiHeight: function(){
		var _this=this;
		var height=$(window).height()-10; //删除padding 10px
	...
	getOnline:function(){
		...
		if(_this.user_ismore==1){
			...
			$.ajax({
				...
				dataType:"json",
				success:function(data){	
					var list=data.list,len=list.length,html='';
					var n=0;
					var liH=38;
					var ulH=liH*len>170? liH*len:170;
					for(var i=0;i<len;i++){
						var userinfo=list[i];
						n=n+1;
						html+='<li data-cardid="'+userinfo.id+'" id="anchor_'+userinfo.id+'" class="anchor"><a class="xinxi" onclick=User.popup("'+userinfo.id+'")>';
						//html+='<span class="user_avatar"><img src="'+userinfo.avatar+'"></span>';
					...
					$(".MR-online .MR-chat .boarder .clearfix").height(ulH);
					_this.resetuserssH();
...
var Video={
	resolution_w:4,
	resolution_h:3,
	plus_w:370, //信息通知区域宽度加margin-left
	plus_h:140, //礼物区域的高度
	stager:$("#LF-stager"), //主体区域
	...
	setHeight: function(){
		...
		var w=height/this.resolution_h*this.resolution_w;
		var stager_w=w+this.plus_w;

		var scrW=$(window).width();  //屏幕宽度
		var navW=$(".SR-nav").width(); //左侧导航宽度
		var chatAreaW=$(".SR-area-chat").outerWidth(true); //右侧聊天区域宽度（带margin）

		var vieoAreaMaxW=scrW-(navW+chatAreaW)-15; //播放器显示区域最大宽度（为了美观去掉15px距离左侧间距）

		if(w>vieoAreaMaxW){
			w=vieoAreaMaxW;
		}

		if(w<this.vieoAreaMinW){
			w=this.vieoAreaMinW;
		}

		var videoHeight=height-this.plus_h;

		if(videoHeight<this.videoMinH){
			videoHeight=this.videoMinH;
		}
		/*if(stager_w>vieoAreaMaxW){
			stager_w=vieoAreaMaxW;
		}*/

		this.LF_video.css({"width":w,"height":videoHeight});
		this.player.css({"width":w,"height":videoHeight});
		...
		$.ajax({
			...
			success:function(data){
				if(data.code!=0){
					...
					// $('#danmu').danmu("addDanmu",{text:chatmsg,color:"white",position:0,size:1,time:time,isnew:1});	
				}			
			},
			error:function(data){
				console.log("出错啦~");
...
var Gift={
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data.errno!=0){
					...
					var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"SendGift","action":"0","ct":"'+data.gifttoken+'","msgtype":"1","level":"'+data.level+'","uid":"'+data.uid+'","timestamp":"'+WlTools.FormatNowDate()+'","uname":"'+_DATA.user.user_nicename+'","uhead":"'+_DATA.user.avatar+'","vip_type":"'+ _DATA.user.vip.type +'","liangname":"'+ _DATA.user.liang.name+'","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}]}'; 
					Socket.emitData('broadcast',msg);
				}
			},
			error: function(data)
	...
	setWidth:function(){
		var videoW=$("#LF-area-video").width();
		var giftListCon=$(".MR-gift .gift-panel .gift-con .con");
		giftListCon.width(videoW-40); //减去左右两侧小箭头
	...
	setAreaShow:function(){
		...
		//获取礼物显示区域的宽度
		var area=$(".MR-gift .gift-panel .gift-con .con");
		var area_li=$(".MR-gift .gift-panel .gift-con .con .scroll .clearfix li");

		var areaW=$(area).width();
		var li_width=$(area_li).width()+2;
		var num=Math.floor(areaW/li_width);

			area_li.each(function(e){
				if(e<num){
					$(this).show();
				}else{
					$(this).hide();
...
var Rank={
	...
	//tabc:$(".MR-rank .con"),
	MR_msg_list:$(".MR-msg-list"),
	nowRank:$(".now_Rank"),
	totalRank:$(".total_Rank"),
	info:$("#LF-area-info"),
	rank:$(".MR-rank"),
	areaTabli:$(".SR-area-Tab li"),
	...
	setHeight: function(){
		...
		//_this.rank.find(".other").css("height",h);		
	},
	/*resetSH: function(){
	...
	switchTab:function(){
		var _this=this;
		this.areaTabli.on("click",function(i){
				var i=$(this).index();
				$(this).addClass("on");
				$(this).siblings().removeClass("on");
				_this.MR_msg_list.find(".tab-con").hide();
				_this.MR_msg_list.find(".tab-con").eq(i).show();
	...
	adddate:function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				now_html+='<div class="thr">'+html_now_t+'</div><div class="MR-rank-more now_more" onmouseover="show_nowOther()" onmouseleave="hide_nowOther()"></div><div class="other now_other" onmouseover="show_nowOther()" onmouseleave="hide_nowOther()"><div class="boards"><ul class="clearfix">'+html_now_o+'</ul></div></div>';

				
				all_html+='<div class="thr">'+html_all_t+'</div><div class="MR-rank-more now_more" onmouseover="show_allOther()" onmouseleave="hide_allOther()"></div><div class="other all_other" onmouseover="show_allOther()" onmouseleave="hide_allOther()"><div class="boards"><ul class="clearfix">'+html_all_o+'</ul></div></div>';
				_this.nowRank.html(now_html);
				_this.totalRank.html(all_html);
				//_this.setScorll();
				//_this.setHeight();
	...
	setThr: function(arr) {
		...
		for (var i = 0; i < len; i++) {
				...
				if(parseFloat(_temp.total)>0){
					...
					_html+='<div class="'+classes[i]+'"><div class="stage">贡献<span class="ICON-coins">'+ _temp.total +'</span></div><div class="photo"><img src="/public/home/show/images/contribute'+(i+1)+'.png" class="rank_bg"><img src="'+_temp.userinfo.avatar+'" data-id="'+_temp.uid+'" class="rank_avatar"></div><p class="name" title="'+_temp.userinfo.user_nicename+'">'+_temp.userinfo.user_nicename+'</p></div>';
				}
...
$(function(){
	...
	$(".left-arrow, .right-arrow").click(function(){
		...
		if (! (!t && 0 >= i || t && i >= $("li", e).size() - 1)) {
			$("li:visible", e).hide();
			var areaW=$(".gift-wrap .con").width();
			var li_width=$(".gift-wrap .con li").width()+2;
			var num=Math.floor(areaW/li_width);
			for (var n = 1; num >= n; n++) {
					var o = t ? i + n: i - n;
					$("li:eq(" + o + ")", e).show()
