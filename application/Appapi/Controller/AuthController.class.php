...
class AuthController extends HomebaseController {
	...
	public function authsave(){
		...
		$data['front_view']=str_replace("http","https",I("front_view") );
		$data['back_view']=str_replace("http","https",I("back_view") );
		$data['handset_view']=str_replace("http","https",I("handset_view") );
		$data['tenant_id']=I("tenant_id");
		$data['addtime']=time();
        $data['status']=0;
		//$authid=M("users_auth")->where("uid='{$data['uid']}'")->getField("uid,status");
        $authid=M("users_auth")->where("uid='{$uid}'")->find();

		if($authid){
		    if($authid['status'] == 0 || $authid['status'] == 1 ){
		      //  var_dump($authid);
                echo json_encode(array("ret"=>0,'data'=>array(),'msg'=>'你已提交身份认证，请勿重复提交！'));exit;
            }else{
