...
class Model_Task extends PhalApi_Model_NotORM
{
    ...
    {
        $list = DI()->notorm->task_classification
        ...
        foreach ($list as $key=>$val){
            ...
            $user_task = DI()->notorm->user_task->where('uid=? and classification=? and status in (1,2)',intval($uid),intval($val['task_classification_id']))->fetchOne();
            $list[$key]['status'] = isset($user_task['status']) ? $user_task['status'] : 0;
            $list[$key]['user_task_id'] = isset($user_task['id']) ? $user_task['id'] : 0;
            $list[$key]['price'] = isset($user_task['price']) ? $user_task['price'] : 0;

            $list[$key]['task_classification_id'] = intval($list[$key]['task_classification_id']);
            $list[$key]['status'] = intval($list[$key]['status']);
            $list[$key]['unlock_amount'] = floatval($list[$key]['unlock_amount']);
            $list[$key]['user_task_id'] = intval($list[$key]['user_task_id']);
            $list[$key]['experience_shop'] = intval($list[$key]['experience_shop']);

            $getTask = $this->getTask($uid,$client,$val['task_classification_id'],'getTaskClassification',$user_info['user_type']);


            if($val['limit_max_balance']>0 && $user_info['coin']>$val['limit_max_balance'] && $user_info['user_type']!=4 && $getTask['code']!=2004){
                continue;
            }
            unset($list[$key]['limit_max_balance']);
            if(count($getTask['info'])>0 || $getTask['code']==2005 || $getTask['code']==2004){ //  '2004' => '分类下有任务未完成', 2005 => 屏蔽游客
    ...
    public function getTask($uid,$client,$task_classification_id,$type='',$user_type='')
    {
        ...
        $classification_info = DI()->notorm->task_classification->where('id=?',intval($task_classification_id))->fetchOne();
        ...
        $user_task = DI()->notorm->user_task->select('id,task_id,status,mtime')->where('uid=? and classification=? and status in (1,2,3)',intval($uid),intval($task_classification_id))->fetchAll();
        $user_task_ids = array();
        if(count($user_task)>0){
            foreach ($user_task as $key=>$val){
                array_push($user_task_ids,$val['task_id']);
                if(in_array($val['status'],[1,2])){
                    if($type != 'getTaskClassification' && !(DI()->notorm->task->where('id=?',$val['task_id'])->fetchOne())){
                        continue;
                    }
    ...
    public function addUserTask($uid, $task_id, $client)
    {
        ...
        $is_exist = DI()->notorm->user_task->where('uid=? and task_id=? and status in(1,2,3)',$uid,$task_id)->fetchOne();
        if($is_exist){
            return array('code' => 2015, 'msg' => codemsg('2015'), 'info' => array()); // 任务已领取
        }

        $task_info = DI()->notorm->task->where('id=?',intval($task_id))->fetchOne();
        if($task_info['start_time'] > time() || $task_info['end_time'] < time()){
            return array('code' => 2016, 'msg' => codemsg('2016'),  'info' => array());  // 当前时间 不在该任务的 任务时间内
        }

        $classification_info = DI()->notorm->task_classification->where('id=?',intval($task_info['classification']))->fetchOne();
        ...
        if($classification_info['experience_shop']=='0' && $classification_info['type']>1 && $task_info['is_upleveltask']=='1'){
            ...
            $usertask_list =  DI()->notorm->user_task->select('task_id')->where('uid=? and task_type=? and status=3 and experience_shop=0',intval($uid),intval($uptype))->fetchAll();
            $usertask_ids = count($usertask_list) > 0 ? array_keys(array_column($usertask_list,null,'task_id')) : [];
            $task_list =  DI()->notorm->task->select('id')->where(
        ...
        if($classification_info['experience_shop']=='0' && $classification_info['daily_task']>0){ // experience_shop：是否体验商城，任务价格大于 0
            $user_task_count = DI()->notorm->user_task->where(
                'uid=? and classification=? and ctime>=? and ctime<=? and status in(1,2,3)',
        ...
        $res = DI()->notorm->user_task->insert($data);
        if(!isset($res['id'])){
    ...
    {
        $user_task_info = DI()->notorm->user_task->where('uid=? and id=?',intval($uid),intval($user_task_id))->fetchOne();
        $task_info = DI()->notorm->task->where('id=?',intval($user_task_info['task_id']))->fetchOne();
        $tenant_id = getTenantId();

        if($user_task_info['status']!=1){
            return array('code' => 2011, 'msg' => '', 'info' => array()); // 状态不是进行中
        }
        ...
        if($task_info['end_time'] < time()){
            ...
            DI()->notorm->user_task->where('uid=? and id=? and status=?',intval($uid),intval($user_task_id),1)->update($data);
        ...
        if($user_task_info['experience_shop']=='1' && $task_info['is_manual_check'] == '0'){ //  experience_shop：是否体验商城，is_manual_check: 人工审核：0否，1是
            ...
            $res = DI()->notorm->user_task->where('uid=? and id=? and status=?',intval($uid),intval($user_task_id),1)->update($user_task_data);
            if($res!==1){
            ...
            if($task_info['only_one']  == 0 ){
                ...
                $user_task_already = DI()->notorm->user_task->where('uid=? and id=? and submit_time  BETWEEN ? and ? ',intval($uid),intval($user_task_id),$start,$end)->fetchOne();
