...
class Upload {
    ...
    private function setDriver($driver = null, $config = null){
        ...
        $UPLOAD_TYPE_CONFIG = C('UPLOAD_TYPE_CONFIG');
        $info = getStorage();
        if($info && isset($info['storage']) && isset($info['storage']['type']) && isset($info['storage'][$info['storage']['type']])){
            $FILE_UPLOAD_TYPE = $info['storage']['type'];
            $UPLOAD_TYPE_CONFIG = array(
                'accessKey' => $info['storage'][$FILE_UPLOAD_TYPE]['accessKey'],
                'secretKey' => $info['storage'][$FILE_UPLOAD_TYPE]['secretKey'],
                'domain' => $info['storage'][$FILE_UPLOAD_TYPE]['domain'],
                'bucket' => $info['storage'][$FILE_UPLOAD_TYPE]['bucket'],
                'upHost' => $info['storage'][$FILE_UPLOAD_TYPE]['upHost'],
            );
        }
    ...
    private function checkExt($ext) {
        return empty($this->config['ext']) ? true : in_array(strtolower($ext), $this->exts);
    }
