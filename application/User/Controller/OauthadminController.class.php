...
class OauthadminController extends AdminbaseController {
	...
	function index(){
		...
		$count=$oauth_user_model->where(array("status"=>1,"tenant_id"=>$tenantId))->count();
		$page = $this->page($count, 20);
		$lists = $oauth_user_model
		->where(array("status"=>1,"tenant_id"=>$tenantId))
	...
	function delete(){
		...
		$rst = M("OauthUser")->where(array("id"=>$id,"tenant_id"=>$tenantId))->delete();
		if ($rst!==false) {
