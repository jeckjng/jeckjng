...
class UsersChargeAdminModel extends AdminModelBaseModel {

    public $table_name = 'users_charge_admin';
    ...
    public function updateUserTypeWithUid($uid, $UserType){
        $res = M($this->table_name)->where(['touid'=>intval($uid)])->save(['user_type'=>intval($UserType)]);
        return $res;
    }
    ...
    public function autoUpdateUserType(){
        ...
        $list = $model->field('id,touid,user_type')->where(['user_type'=>0])->limit(1000)->order('id desc')->select();
        $count = 0;
        foreach ($list as $key=>$val){
            if($val['user_type'] == '0' && !$redis->hGet('user_type_is_empty',$val['uid'])){
                $user_info = getUserInfo($val['touid']);
