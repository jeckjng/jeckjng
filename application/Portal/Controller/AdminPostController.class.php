...
class AdminPostController extends AdminbaseController {
	...
	private  function _lists($status=1){
        ...
        foreach ($posts as $key=>$val){
            array_push($uids,$val['post_author']);
        }
        if(count($uids)>0){
            $uids = implode(',',$uids);
            $userlist = M("Users")->field("id,user_login")->where("id in(".$uids.")")->select();
        }
        $userlist = isset($userlist) ? array_column($userlist,'user_login','id') : [];

        foreach ($posts as $key=>$val){
            $posts[$key]['user_login'] = isset($userlist[$val['post_author']]) ? $userlist[$val['post_author']] : '';
        }
	...
	function delete(){
		...
		if(isset($_POST['ids'])){
			...
			if ($this->term_relationships_model->where("tid in ($tids)")->delete()) {
                $action="删除文章：{$tids}";
                    setAdminLog($action);
				$this->success("删除成功！");
			} else {
