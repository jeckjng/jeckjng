...
class PromotersController extends AdminbaseController{
			...
			if(!empty($_POST['role_id']) && is_array($_POST['role_id'])){
				...
				if ($this->users_model->create()) {
					...
					if ($result!==false) {
                        ...
                        $path=setpath($one_id);
                        $isexist=M('users_agent')->where("uid={$one_id}")->find();
                        if($isexist){
                            $path=$isexist['path'].$path;
                        }
                        M('users_agent')->add(array("uid"=>$result,"type"=>'1',"path"=>$path,'addtime'=>time()));
	...
	function edit(){
        ...
        $proxyinfo=M('users_agent')->where("uid={$id}")->find();
			...
			if(!empty($_POST['role_id']) && is_array($_POST['role_id'])){
				...
				if ($this->users_model->create()) {
					...
					if ($result!==false) {
                        ...
                        M('users_agent')->where("uid={$uid}")->save( array("path"=>$path) );
