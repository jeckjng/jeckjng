...
class PushController extends AdminbaseController {
    ...
    function del(){
        ...
        $tenantId=getTenantIds();
        if($id){
            $result=M("pushrecord")->where("id=%d and tenant_id=%d",$id,$tenantId)->delete();
                if($result){
                    $action="删除推送信息：{$id}";
                    setAdminLog($action);
                        $this->success('删除成功');
...
//                        file_put_contents( $_SERVER['DOCUMENT_ROOT'].'/data/Log/jpush.txt',date('y-m-d h:i:s').'提交参数信息 设备名:'.json_encode($alias)."\r\n",FILE_APPEND,FILE_APPEND);
//                        file_put_contents( $_SERVER['DOCUMENT_ROOT'].'/data/Log/jpush.txt',date('y-m-d h:i:s').'提交参数信息:'.$e."\r\n",FILE_APPEND,FILE_APPEND);
                    }					
                }
            }
            /* 极光推送 */
