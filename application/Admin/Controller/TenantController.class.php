...
class TenantController extends AdminbaseController {

    private $goGroup = '/tenant';
    private $thirdTenantName = 'meibo';
    ...
    public function add_post(){
        if(IS_POST) {
            ...
            if($param['balance_query_url']==""){
                $this->error("余额查询接口地址不能为空！");
            }
            if($param['balance_update_url']==""){
                $this->error("余额更新接口地址不能为空！");
            }
        ...
        if(IS_POST) {
            ...
            if(enableGolangReplacePhp() === true){
                ...
                $rules = array(
                    ...
                    array('balance_query_url', 'require', '余额查询接口地址不能为空！'),
                    array('balance_update_url', 'require', '余额更新接口地址不能为空！')
                );
                ...
                //更新租户的域名和管理员密码
                M("tenant_config")->where("tenant_id=%d", $tenant->id)->setField("site", $tenant->site);
