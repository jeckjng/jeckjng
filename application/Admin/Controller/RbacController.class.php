...
class RbacController extends AdminbaseController {
    ...
    public function index() {
        ...
        $data = $this->role_model->order(array("listorder" => "asc", "id" => "desc"))->limit($page->firstRow . ',' . $page->listRows)->select();
    		...
    		if (is_array($_POST['menuid']) && count($_POST['menuid'])>0) {
    			...
    			foreach ($_POST['menuid'] as $menuid) {
    				...
    				if($menu){
    					...
    					// 防止重复添加
    					$hash_key = md5($roleid.$name.'admin_url');
    					if(in_array($hash_key, $auth_access_list)){
    					    continue;
                        }
    					array_push($auth_access_list, $hash_key);
    			...
    			setAdminLog($action);
                $redis->hDel('auth_access_list', $roleid);
