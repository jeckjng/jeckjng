var Login={
	...
	app_url:'/index.php?m=App&a=programe',
	dombody:$("body"),
	type:'login',
					...
					reg_html='<div class="login_pop js_reg_pop">\
						...
						var signin='</article>\
											...
											<article>\
												...
												<div class="phoneArea">\
													<i class="phoneIcon"></i>\
													<input class="agent_code js_reg_agent_code_input" type="text" placeholder="邀请码（选填）">\
													<input class="game_tenant_id js_reg_game_tenant_id_input" type="hidden">\
													<input class="game_tenant_id js_reg_zone_input" type="hidden">\
												</div>\
	...
	addEvent:function(){
		...
		this.js_reg_agent_code_input=$(".js_reg_agent_code_input"),
	...
	regVerify:function(){
		...
		function getCodeU(){
			$.ajax({
				url: _this.code_url,
				data: {mobile:phone.val(),captcha:captcha.val(),type:'reg'},
				type: "GET",
				dataType: "jsonp",
				jsonp: "callback",
				cache: !1,
	...
	doreg:function(){
		var _this=this,
				...
				agent_code =this.js_reg_agent_code_input.val(),
				game_tenant_id =$(".js_reg_game_tenant_id_input").val();
				zone =$(".js_reg_zone_input").val();
		$.ajax({
			url: _this.reg_url,
			data: {mobile:phone,pass:pass,code:code,agent_code:agent_code,game_tenant_id:game_tenant_id,zone:zone},
			type: "GET",
			dataType: "jsonp",
			jsonp: "callback",
			cache: !1,
			success:function(data){
				if(data && data.errno ==0){
					...
					layer.msg("注册成功！",{},function(){
						window.location.href = _this.app_url; //跳转到app下载页面
					});
				}else{
					_this.loginWarring(data.errmsg);
					return !1;
	...
	forgetVerify:function(){
		...
		function getCodeU(){
			$.ajax({
				url: _this.code_url,
				data: {mobile:phone.val(),captcha:captcha.val(),type:'forget'},
				type: "GET",
				dataType: "jsonp",
				jsonp: "callback",
				cache: !1,
...
$(function(){
	...
	$.fn.follow = function(e, r, i) {
		...
		var t = '/index.php?g=home&m=user&a=follow_',
			o = i || n[r];
		$.ajax(t + r, {
			dataType: "jsonp",
			data: {
