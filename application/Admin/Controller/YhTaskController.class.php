...
class YhTaskController extends AdminbaseController {
    ...
    public function classification_list(){
        $classification_list = M("yh_task_classification")->field('id,name,type')->order('id asc')->select();
        $classification_list = array_column($classification_list,null,'id');
        return $classification_list;
    }

    public function user_task_status_list(){
        return array('0'=>'已取消','1'=>'进行中','5'=>'任务待提交','2'=>'审核中','3'=>'审核通过','4'=>'审核拒绝');
    }

    public function reward_type(){
        return array('1'=>'金额','2'=>'转盘次数');
    }
    ...
    function index(){
        ...
        $this->assign('reward_type', $this->reward_type());
    ...
    * */
    function add_task(){
        if(IS_POST){
            ...
            if($param['reward1_number']<0){
                $this->error('完成奖励2 次数不规范');
            }

            $classification_info = M("yh_task_classification")->where(['id'=>intval($param['classification'])])->find();
            ...
            $result = M('yh_task')->where(['classification'=>intval($param['classification'])])->find();
            if ($result){
                $this->error('同一任务分类只允许添加一个任务，请更换任务分类');
            }
            ...
            $data = array(
                ...
                //'reward_type' => intval($param['reward_type']),
                'reward1_number' => intval($param['reward1_number']),
            );
    ...
    * */
    function edit_task(){
        if(IS_POST){
            ...
            $oldTask = M('yh_task')->where(['id'=>$param['id']])->find();
            if ($oldTask['classification'] != $param['classification']){
                $result = M('yh_task')->where(['classification'=>intval($param['classification'])])->find();
                if ($result){
                    $this->error('同一任务分类只允许添加一个任务，请更换任务分类');
                }
            ...
            M('yh_task')->where(['id'=>$param['id']])->save($data);
            $this->success('操作成功',U('index'));
        }

        $info = M('yh_task')->where(['id'=>I('id')])->find();
    ...
    * */
    function del_task(){
        ...
        if($param['id']){
            ...
            M('yh_task')->where(['id'=>$param['id']])->delete();
            $this->success('删除成功');
        }
        $this->error('删除失败');
    }
    ...
    * */
    function classification(){
        ...
        $task_classification=M("yh_task_classification");
    ...
    * */
    function get_classification_with_type(){
        ...
        $lists = M("yh_task_classification")->field('id,name')->where(['type'=>intval(I('type'))])->order("id desc")->select();
    ...
    function del_classification(){
        ...
        if($param['id']){
            ...
            M('yh_task_classification')->where(['id'=>$param['id']])->delete();
            $this->success('删除成功');
        }
        $this->error('删除失败');
    }
    ...
    function addedit_classification(){
        if(IS_POST){
            ...
            if(isset($param['id']) && $param['id']>0){
                M('yh_task_classification')->where(['id'=>$param['id']])->save($data);
            }else{
                M('yh_task_classification')->add($data);
            }
        ...
        $info = [];
        if(I('id')){
            $info = M('yh_task_classification')->where(['id'=>I('id')])->find();
        }
    ...
    * */
    function user_task(){
        ...
        if(I('uid')){
            $map['cmf_yh_user_task.uid'] = I('uid');
        }
        if(I('user_login')){
            $u_info = M("users")->where(['user_login'=>I('user_login'),'user_type'=>['in',[2,5,6]]])->field('id,user_login')->find();
            $map['cmf_yh_user_task.uid'] = $u_info['id'];
        }
        $map['cmf_yh_user_task.ctime'] = array("between",array(strtotime($start_time),strtotime($end_time)?strtotime($end_time):time()));
        if (I('classification')) {
            $map['cmf_yh_user_task.classification'] = I('classification');
        }
        if(I('status')!=''){
            $map['cmf_yh_user_task.status'] = I('status');
        }

        $user_task=M('yh_user_task');
        ...
        foreach($lists as $key=>$val){
            ...
            $lists[$key]['task_name'] = M("yh_task")->where(['id'=>intval($val['task_id'])])->getField('name');
   ...
   * */
    function view_rewardlog(){
        ...
        $role_id=$_SESSION['role_id'];
        if($role_id==1){
            $lists = M('yh_task_rewardlog')->where($map)->order("id desc")->select();
        }else{
            $map['tenant_id'] = $tenantId;
            $lists = M('yh_task_rewardlog')->where($map)->order("id desc")->select();
        }
        ...
        $client_list = $this->client_list();
        foreach($lists as $key=>$val){
            $usertask_info = M('yh_user_task')->where(['id'=>$val['user_task_id']])->find();
            $classifi_info = M("yh_task_classification")->where(['id'=>$usertask_info['classification']])->find();
            $lists[$key]['userinfo'] = getUserInfo($val['uid']);
            if($val['vip_id']==0){
    ...
    function cancel_task(){
        $param = I('param.');
        if($param['id']){
            M('yh_user_task')->where(['id'=>$param['id'],'status'=>1])->save(['status'=>0]);
            $this->success('取消任务成功');
        }
        $this->error('取消任务失败');
    }
    ...
    * */
    function immediately_check(){
        if(IS_POST){
            ...
            $info = M('yh_user_task')->where(['id'=>intval(I('id'))])->find();
            if ($info['only_one']==0) {
                if ($info['status'] != 2) {
            ...
            if($param['status'] == 4){
                try{
                    ...
                    M('yh_user_task')->where(['id'=>$param['id']])->save($data);
                ...
                try{
                    ...
                    if($info['reward1']>0){  // 完成奖励1 大于 0 才执行
                        $awardLog = [
                            ...
                            'type' => 1,
                            'amount' => $reward1,
                            'original_balance' =>  $user_info['coin'],
                            'back_balance' =>  bcadd($user_info['coin'],$reward1 ,2),
                            'completion_value'=> 0,
                            'addtime' => time(),
                            'status' => 1,
                            'data_type' => 1,
                            'award_name' => '新手任务-'.$info['task_name'],
                        ];
                        M('award_log')->add($awardLog);

                        M('users')->execute('UPDATE '.C('DB_PREFIX').'users SET coin=coin+'.floatval($reward1). ' WHERE id = '.intval($info['uid']));
                        delUserInfoCache($info['uid']);
                        $user_info = UsersModel::getInstance()->getUserInfoWithIdAndTid($info['uid']);
                        // 任务奖励
                        ...
                        M('yh_task_rewardlog')->add($task_rewardlog_data);
                    }

                    $reward1_number = intval($info['reward1_number']);
                    if($reward1_number>0){  // 完成奖励3 大于 0 才执行
                        M('users')->where(['id'=>intval($info['uid'])])->setInc('turntable_times',$reward1_number);
                        delUserInfoCache($info['uid']);

                        $awardLog = [
                            'uid' => $info['uid'],
                            'type' => 1,
                            'amount' => $reward1_number,
                            'original_balance' =>  $user_info['turntable_times'],
                            'back_balance' =>  bcadd($user_info['turntable_times'],$reward1_number ,2),
                            'completion_value'=> 0,
                            'addtime' => time(),
                            'status' => 1,
                            'data_type' => 3,
                        ...
                        M('yh_task_rewardlog')->add($task_rewardlog_data); // 奖励明细记录
                    }
                    ...
                    if($info['reward2_upgrade_vip']=='1'){ // 升级VIP等级
                        ...
                        M('yh_task_rewardlog')->add($task_rewardlog_data1);
                    }
        ...
        $info['task_name'] = M("yh_task")->where(['id'=>intval($info['task_id'])])->getField('name');
        $info['classification'] = M("yh_task_classification")->where(array('id'=>$info['classification']))->getField('name');
    ...
    * */
    function reward_log(){
        ...
        $task_rewardlog=M('yh_task_rewardlog');
        $tenantId=getTenantIds();
        $role_id=$_SESSION['role_id'];
        if($role_id==1){
        ...
        $type_list = $this->type_list();
        $reward_type_list = ['1'=>'奖励1','2'=>'奖励2','3'=>'奖励3'];
        foreach($lists as $key=>$val){
            $lists[$key]['userinfo'] = getUserInfo($val['uid']);
            if($val['vip_id']==0){
                $vip_info = M("vip")->where(array('orderno'=>1))->order('length asc')->limit(1)->find();
            ...
            $lists[$key]['type'] = $type_list[$val['type']];
            $lists[$key]['task_name'] = M("yh_task")->where(array('id' =>$val['task_id'] ))->getField('name');
    ...
    * */
    function login_reward(){
        if(IS_POST){
            ...
            if(!M('yh_task_loginreward')->where(['type'=>1])->find()){
                $data['type'] = 1;
                M('yh_task_loginreward')->add($data);
            }else{
                M('yh_task_loginreward')->where(['id'=>$param['id']])->save($data);
            }
            $this->success('操作成功');
        }

        $info = M('yh_task_loginreward')->where(['type'=>1])->find();
