...
class AdminTermController extends AdminbaseController {
	...
	function index(){
		$count = $this->terms_model->count();
		$page = $this->page($count);
		$result = $this->terms_model->order(array("listorder"=>"asc"))->limit($page->firstRow . ',' . $page->listRows)->select();
		$tree = new \Tree();
		$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		foreach ($result as $r) {
			...
			$r['str_manage'] = '<a href="' . U("AdminTerm/edit", array("id" => $r['term_id'])) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete del_color" href="' . U("AdminTerm/delete", array("id" => $r['term_id'])) . '">'.L('DELETE').'</a> ';
