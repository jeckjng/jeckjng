...
class RegController extends HomebaseController {
	...
	function reg(){
        ...
        if($agentid){
            $agentuid=M('users_agent_code')->where("code='{$agentid}'")->getField('uid');
            if(!$agentuid){
                $this->assign("reason",'邀请码不正确');
                $this->display(':error');
                exit;
	...
	public function wxLogin(){
		...
		$callback  = get_upload_path('/wxshare/index.php/Share/wxLoginCallback?agentid='.$agentid); //回调地址
		//微信登录
		//session_start();
		//-------生成唯一随机串防CSRF攻击
    ...
    public function regdo(){
        ...
        if($agentuid){
            $agent=M('users_agent');
            $agentinfo=$agent->where("uid={$agentuid}")->find();
            //var_dump($agentinfo);
            if($agentinfo){                
                $data2=[
                    'uid'=>$userid,
