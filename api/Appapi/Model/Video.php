    ...
    public function delMyVideo($uid,$videoid,$type){
	    if ($type == 1){
            ...
            if($videoInfo['origin'] == 1){
                Cache_Video::getInstance()->delPrivateListIdCache($videoInfo['tenant_id'], $videoInfo['id']);
            }
            if(in_array($videoInfo['origin'], [2,3])) {
                Cache_Video::getInstance()->delPublicListIdCache($videoInfo['tenant_id'], $videoInfo['id']);
            }
            if($videoInfo['top'] == 1){
                Cache_Video::getInstance()->delTopListIdCache($videoInfo['tenant_id'], $videoInfo['id']);
                Cache_Video::getInstance()->delTenantTopListCache($videoInfo['tenant_id']);
            }
            if($videoInfo['is_advertise'] == 1){
                Cache_Video::getInstance()->delAdvertiseListIdCache($videoInfo['tenant_id'], $videoInfo['id']);
                Cache_Video::getInstance()->delTenantAdvertiseListCache($videoInfo['tenant_id']);
            }
        }else{
