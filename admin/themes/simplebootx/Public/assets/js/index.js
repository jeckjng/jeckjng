...
function openapp(url, appid, appname, refresh) {
    ...
    if ($app.length == 0) {//console.log(url, appid, appname, refresh)
    ...
    var app_id = $('#breadcrumbs .current').attr('app-id');
    var left_app_id = $('.' + app_id + '');
    var li = left_app_id.closest('.nav-list').find('.submenu');
    li.each(function(index, item) {
        $(item).hide();
    });
    left_app_id.parent().parent().show();
    left_app_id.parent().parent().parent().addClass('open');
    left_app_id.parent().parent().parent().siblings().removeClass('open');
    left_app_id.css('color', '#1abc9c');
